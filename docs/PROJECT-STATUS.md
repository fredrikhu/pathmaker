...
**Prior — Paladin and ranger spell lists authored; the caster gap batch 2 exposed is closed.** Widening SpellList to include 'paladin'/'ranger' had left both classes resolving to almost no spells because nothing carried those lists. Both are now applied like the witch list — PALADIN_LEVELS and RANGER_LEVELS maps parsed from each class's own d20pfsrd page, overlaid to add the list tag and a per-list level override where it differs from the spell's flat level (the overlay loop that was witch-only is now generalised over the three). Nearly every spell already existed; only Heal Mount was new (paladin 3), and Bless Weapon's flat level was corrected 2 → 1. The two lists disagree on shared spells and the overlay gets it right (Cure Light Wounds paladin 1 / ranger 2; Neutralize Poison paladin 4 / ranger 3); the paladin gets only the chaos/evil alignment spells. A content test audits both lists (45 paladin, 51 ranger, levels 1–4, the alignment split). 831 tests; browser-verified a paladin 12 and ranger 12 preparing their level-correct spells.

**Prior — Spell catalogue: the Core Rulebook set is COMPLETE (batch 4, levels 7–9), 566 → 620.** 609 of the 610 distinct CRB spells across the four class lists are now carried; the only omission is Blot, which d20pfsrd's bard table sources to PZO1110 but whose own page credits Goblins of Golarion. 54 spells this batch — the arcane 7–9 capstones, the divine mass/symbol/summon lines, the druid's own, and the tops of the numbered polymorph/summon ladders. The whole-catalogue audit surfaced `elemental-body-ii`, which had fallen between batches 2 (i) and 3 (iii). A content test now asserts coverage is complete but for Blot, so a future gap in the Core set fails the suite. 827 tests; browser-verified a level-20 wizard's 8th and 9th bands.

**Prior — Spell catalogue: CRB completion batch 3 (levels 5–6), 494 → 566 (555 of 610 CRB spells covered; 55 remain at 7–9).** 73 spells, every field verified on the spell's own d20pfsrd page. The whole-catalogue audit test paid off again: the mass stat-buffs inherit their single-target lists, and it caught two transpositions before commit — cat's grace mass is druid not divine, eagle's splendor mass is divine not druid. Eight spells needed `levelByList` (the bard reaches greater scrying/shout, mass charm monster, irresistible dance and project image ahead of the wizard). 824 tests; browser-verified a level-12 wizard's 6th band and a level-12 druid's 5th/6th bands, with the wizard-8 and druid-7 spells correctly absent from the lower bands.
...
- **Weapons**: 80 weapons (verified against d20pfsrd — damage/crit/type/range/cost/weight),
  ...
  verified against d20pfsrd; splatbook feats remain out of scope. **Spells**: 620 across levels 0–9,
  covering 609 of the 610 distinct Core Rulebook spells on the four class lists — the CRB set is
  **complete** (the sole omission is Blot, a splatbook spell d20pfsrd mis-sources to the CRB). Every list membership and
...
column, so a first-column scrape reports every option missing. Left out everywhere: traits gated on
  ...
  **Source powers can now carry numbers** (2026-09-21): `SourceFeature.effectsAt(classLevel)` is
  ...
  repeats them. Enchanting Smile (+2 enhancement on Bluff/Diplomacy/Intimidate, +1 per 5 levels,
  max +6) uses the same hook. **Equipment**: the full Core armor table (hide, splint/banded/half-plate, steel
