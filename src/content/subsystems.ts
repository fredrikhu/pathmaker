...
import type { CompanionAttackDef, EvolutionDef } from './model';
...
/** A natural attack an evolution grants, written at Medium size — the engine steps the die up if
 *  the eidolon is Large, exactly as each evolution's own text does. */
const a = (name: string, count: number, damage: string, extra: Partial<CompanionAttackDef> = {}): CompanionAttackDef =>
...
export const EIDOLON_EVOLUTIONS: EvolutionDef[] = [
  ...
  ev('bite', 'Bite', 1, 'A primary bite attack (1d6, 1d8 if Large).',
    { apply: { attacks: [a('bite', 1, '1d6')] } }),
  ev('claws', 'Claws', 1, 'Two primary claw attacks (1d4, 1d6 if Large). Requires the limbs (legs) evolution.',
    { multi: true, apply: { attacks: [a('claw', 2, '1d4')] } }),
  ev('climb', 'Climb', 1, 'A climb speed equal to base speed (+20 ft each extra time taken).',
    { multi: true, apply: { speed: { climb: 'base' } } }),
  ev('gills', 'Gills', 1, 'Breathe underwater indefinitely.',
    { apply: { special: ['Breathes underwater'] } }),
  ev('improved-damage', 'Improved Damage', 1, "Increase one natural attack's damage die by one step.",
    { multi: true, apply: { manual: true } }),
  ev('improved-natural-armor', 'Improved Natural Armor', 1, '+2 natural armor. May be taken once per five summoner levels.',
    { multi: true, apply: { naturalArmor: 2 } }),
  ev('magic-attacks', 'Magic Attacks', 1, 'Natural attacks count as magic for overcoming DR.',
    { apply: { special: ['Natural attacks count as magic'] } }),
  ev('mount', 'Mount', 1, 'Serve as a combat-trained mount (must be a size larger than its rider).',
    { forms: ['quadruped', 'serpentine'], apply: { special: ['Combat-trained mount'] } }),
  ev('pincers', 'Pincers', 1, 'Two secondary pincer attacks (1d6, 1d8 if Large). Requires limbs (arms).',
    { multi: true, apply: { attacks: [a('pincer', 2, '1d6', { secondary: true })] } }),
  ev('pounce', 'Pounce', 1, 'Make a full attack at the end of a charge.',
    { forms: ['quadruped'], apply: { special: ['Pounce'] } }),
  ev('pull', 'Pull', 1, 'One attack type can drag foes 5 ft closer.', { multi: true, apply: { manual: true } }),
  ev('push', 'Push', 1, 'One attack type can shove foes 5 ft away.', { multi: true, apply: { manual: true } }),
  ev('reach', 'Reach', 1, "One attack's reach increases by 5 ft.", { apply: { manual: true } }),
  ev('resistance', 'Resistance', 1, 'Energy resistance 5 to one energy type (10 at higher pools).',
    { multi: true, apply: { manual: true } }),
  ev('scent', 'Scent', 1, 'Gain the scent ability.', { apply: { senses: ['scent'] } }),
  ev('skilled', 'Skilled', 1, '+8 racial bonus to one skill.', { multi: true, apply: { manual: true } }),
  ev('slam', 'Slam', 1, 'A primary slam attack (1d8, 2d6 if Large). Requires limbs (arms).',
    { multi: true, apply: { attacks: [a('slam', 1, '1d8')] } }),
  ev('sting', 'Sting', 1, 'A primary sting attack (1d4, 1d6 if Large). Requires the tail evolution.',
    { multi: true, apply: { attacks: [a('sting', 1, '1d4')] } }),
  ev('swim', 'Swim', 1, 'A swim speed equal to base speed (+20 ft each extra time taken).',
    { multi: true, apply: { speed: { swim: 'base' } } }),
  ev('tail', 'Tail', 1, '+2 racial bonus on Acrobatics to balance.',
    { multi: true, apply: { special: ['+2 Acrobatics to balance (tail)'] } }),
  ev('tail-slap', 'Tail Slap', 1, 'A secondary tail slap attack (1d6, 1d8 if Large). Requires the tail evolution.',
    { multi: true, apply: { attacks: [a('tail slap', 1, '1d6', { secondary: true })] } }),
  ev('tentacle', 'Tentacle', 1, 'A secondary tentacle attack (1d4, 1d6 if Large).',
    { multi: true, apply: { attacks: [a('tentacle', 1, '1d4', { secondary: true })] } }),
  ev('wing-buffet', 'Wing Buffet', 1, 'Two secondary wing buffet attacks (1d4, 1d6 if Large). Requires flight with wings.',
    { apply: { attacks: [a('wing buffet', 2, '1d4', { secondary: true })] } }),
  // 2-point
  ev('ability-increase', 'Ability Increase', 2, '+2 to one ability score.', { multi: true, apply: { manual: true } }),
  ev('constrict', 'Constrict', 2, 'Deal extra damage when grappling with grab.',
  ...
  ev('immunity', 'Immunity', 2, 'Immunity to one energy type.', { minLevel: 7, multi: true, apply: { manual: true } }),
  ev('limbs', 'Limbs', 2, 'An extra pair of limbs. A pair of legs adds 10 ft to base speed; arms grant no attack of their own.',
    { multi: true, apply: { speed: { baseBonus: 10 } } }),
  ev('poison', 'Poison', 2, 'A bite or sting delivers poison (Con damage).', { minLevel: 7, apply: { manual: true } }),
  ev('rake', 'Rake', 2, 'Two primary rake attacks (1d4) against grappled foes.',
    { minLevel: 4, forms: ['quadruped'], apply: { special: ['Rake 1d4 (grappled foes only)'] } }),
  ev('rend', 'Rend', 2, 'Two hits with claws deal an extra bonus of damage.', { minLevel: 6, apply: { special: ['Rend'] } }),
  ev('trample', 'Trample', 2, 'Overrun smaller foes for damage as a full-round action.',
    { forms: ['biped', 'quadruped'], apply: { special: ['Trample'] } }),
  ev('tremorsense', 'Tremorsense', 2, 'Tremorsense 30 ft.', { minLevel: 7, apply: { senses: ['tremorsense 30 ft'] } }),
  ev('trip', 'Trip', 2, 'A successful bite can trip the target.', { apply: { special: ['Bite can trip'] } }),
  ev('weapon-training', 'Weapon Training', 2, 'Gain Simple Weapon Proficiency (martial for +2 more points).',
    { apply: { special: ['Simple weapon proficiency'] } }),
  // 3-point
  ev('blindsense', 'Blindsense', 3, 'Blindsense 30 ft.', { minLevel: 9, apply: { senses: ['blindsense 30 ft'] } }),
  ev('burrow', 'Burrow', 3, 'A burrow speed equal to half base speed.',
    { minLevel: 9, apply: { speed: { burrow: 'half' } } }),
  ev('damage-reduction', 'Damage Reduction', 3, 'DR 5 bypassed by one alignment (10 at 15th+).',
    { minLevel: 9, apply: { manual: true } }),
  ev('frightful-presence', 'Frightful Presence', 3, 'Frightful presence that can shake or panic foes.',
    { minLevel: 11, apply: { special: ['Frightful presence'] } }),
  ev('swallow-whole', 'Swallow Whole', 3, 'Swallow a grappled foe smaller than itself.',
    { minLevel: 9, apply: { special: ['Swallow whole'] } }),
  ev('web', 'Web', 3, 'Spin webs to entangle foes.', { minLevel: 7, apply: { special: ['Web'] } }),
  // 4-point
  ev('blindsight', 'Blindsight', 4, 'Blindsight 30 ft.', { minLevel: 11, apply: { senses: ['blindsight 30 ft'] } }),
  ev('breath-weapon', 'Breath Weapon', 4, 'A cone or line breath weapon of one energy type.',
    { minLevel: 9, apply: { manual: true } }),
  ev('fast-healing', 'Fast Healing', 4, 'Fast healing 1 while it has 1+ hp.',
    { minLevel: 11, apply: { special: ['Fast healing 1'] } }),
  ev('large', 'Large', 4, 'Grow to Large: +8 Str, +4 Con, +2 natural armor, −2 Dex, −1 AC and attack, +1 CMB/CMD.',
    { minLevel: 8, apply: { size: 'large', abilities: { str: 8, con: 4, dex: -2 }, naturalArmor: 2 } }),
  ev('spell-resistance', 'Spell Resistance', 4, "SR equal to 11 + the summoner's level.",
    { minLevel: 9, apply: { special: ['Spell resistance 11 + summoner level'] } }),
];
